...
tCmdLineEntry g_psCmdTable[] =
{
    ...
    { "lock",           Cmd_lock,           "Locks the interface, disabling commands except for NULL, RREG, and UNLOCK" },
    { "unlock",         Cmd_unlock,         "Unlocks the interface, enabling all commands" },
...
int Cmd_debug(const int argc, const char *argv[])
{
    ...
    PRINT("CRC_TYPE :\t %s", (SPI_CRC_TYPE ? "ANSI" : "CCITT"));
...
int Cmd_id(const int argc, const char *argv[])
{
    ...
    char device_id[] = "ADS131B0x-Q1";
    device_id[8] = (char) (4 + '0');
...
int Cmd_wreg(const int argc, const char *argv[])
{
    ...
    if (checkDeviceLocked()) {return(CMDLINE_DEVICE_LOCKED); }
...
int Cmd_regmap(const int argc, const char *argv[])
{
    ...
    // TODO: For ADS131B04-Q1, do NOT read registers 29-62, need to skip these here...
...
#if 1
    // SLOWEST DATA READ IMPLEMENTATION

    // Read data
    adc_channel_data chData;
    uint32_t i;
    for(i = 0; i < g_num_samples; i++)
    {
...
int Cmd_crc(const int argc, const char *argv[])
{
    ...
    // Print response
    PRINT("%s CRC equals \"0x%04X\"", (SPI_CRC_TYPE ? "ANSI" : "CCITT"), crc);
...
int Cmd_null(const int argc, const char *argv[])
{
    ...
    // Print response
    PRINT("Command Response: 0x%04X", value);
...
int Cmd_lock(const int argc, const char *argv[])
{
    ...
    // Send LOCK command
    lockRegisters();
    PRINT("Locking the device. Accepted commands are NULL, RREG, and UNLOCK");
...
bool checkDeviceLocked(void)
{
    ...
    {
        // Print error message
        PRINT("Error: Device locked, command not accepted. Accepted commands are NULL, RREG, and UNLOCK");
        return (1);
    }
...
void PrintLogo(void)
{
    ...
    PRINT("   \\.  _} ADS131B04-Q1EVM Console Application\r\n");
    PRINT("     \\(   v1.0.0, (C) 2021\r\n\n");
    PRINT("----------------------------------\n");
